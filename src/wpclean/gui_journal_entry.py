...
_JOURNAL_JS = r'''
state.detailTab=state.detailTab||'progress';state.currentProject=null;state.confirmationRevealKey=state.confirmationRevealKey||'';
...
function progressTabHtml(p){return `<div class="drawer-body"><div class="detail-layout"><div class="detail-left"><div class="section"><div class="section-head"><div class="section-title">Kết nối FTP</div></div>${connectionHtml(p)}</div><div class="section"><div class="section-head"><div class="section-title">Tiến độ xử lý</div></div><div class="steps">${stepHtml(p)}</div>${jobHtml(p)}${decisionHtml(p)}</div>${p.themeRepair?`<div class="section"><button class="btn btn-warning" onclick="openRepair('${esc(p.name)}')">Mở thư mục theme repair</button></div>`:''}${p.completed?`<div class="foot-danger"><button class="btn btn-danger" onclick="deleteProject('${esc(p.name)}')">Xóa dự án local</button></div>`:''}</div><div class="detail-right">${terminalPanelHtml(p)}</div></div></div>`}
...
renderDrawer=function(p){state.selected=p.name;state.currentProject=p;const key=confirmationKey(p),reveal=!!key&&key!==state.confirmationRevealKey;if(reveal)state.detailTab='progress';if(!key&&state.confirmationRevealKey.startsWith((p.name||'')+'|'))state.confirmationRevealKey='';const pc=progressOf(p),si=statusInfo(p);let body=state.detailTab==='history'?historyHtml(p):state.detailTab==='todos'?todoHtml(p):progressTabHtml(p);qs('#drawerContent').innerHTML=`<div class="drawer-head"><div class="drawer-title"><div class="drawer-title-main"><h2>${esc(p.host)}</h2><div class="drawer-project-meta"><span class="project-token">${esc(p.name)}</span><span class="status ${si[0]}">${si[1]}</span><span class="workflow-token">${pc}% workflow</span>${p.pendingTodoCount?`<span class="workflow-token">${p.pendingTodoCount} việc chờ</span>`:''}</div></div><button class="xbtn" type="button" aria-label="Đóng chi tiết dự án" onclick="closePanels()">×</button></div></div>${detailTabs(p)}${body}`;if(reveal){state.confirmationRevealKey=key;requestAnimationFrame(revealConfirmation)}else if(state.detailTab==='progress'&&state.logFollow)setTimeout(scrollTerminalBottom,0)}
