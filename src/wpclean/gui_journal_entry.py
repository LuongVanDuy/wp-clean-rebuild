...
.drawer{top:16px;right:16px;bottom:16px;width:min(1500px,calc(100vw - 32px));border:1px solid #2b3950;border-radius:8px;background:#0a111e;box-shadow:-18px 0 55px rgba(0,0,0,.38)}.drawer-head{min-height:var(--drawer-head-height);padding:12px 20px;background:#0c1524}.drawer-title{align-items:center}.drawer-title-main{min-width:0}.drawer-project-meta{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:7px}.project-token,.workflow-token{display:inline-flex;align-items:center;min-height:24px;border:1px solid #34445b;border-radius:4px;background:#0a1220;padding:3px 7px;color:#9fb0c5;font:11px/1.2 var(--mono)}.detail-tabs{top:var(--drawer-head-height);background:#0c1524}.drawer-body{padding:18px 20px 24px}.detail-layout{grid-template-columns:minmax(470px,.82fr) minmax(650px,1.18fr);gap:18px}.detail-left,.detail-right{min-width:0}.detail-left>.section{margin-bottom:16px}.detail-right{top:calc(var(--drawer-head-height) + 48px)}.terminal-head{min-height:54px}.terminal-tools{display:flex;align-items:center;gap:7px}.terminal-output{height:min(430px,calc(100vh - 250px));min-height:320px;max-height:430px}.foot-danger{justify-content:flex-end;margin-top:16px;padding-top:14px}.action-card.confirmation-reveal{border-color:#fbbf24;box-shadow:0 0 0 3px rgba(251,191,36,.16),0 12px 28px rgba(0,0,0,.2);outline:0}
...
function projectExtrasHtml(p){const deleting=!!p.localDeletion?.status;return `${p.themeRepair&&!deleting?`<div class="section"><button class="btn btn-warning" onclick="openRepair('${esc(p.name)}')">Mở thư mục theme repair</button></div>`:''}${p.completed&&!deleting?`<div class="foot-danger"><button class="btn btn-danger" onclick="deleteProject('${esc(p.name)}')">Xóa dự án local</button></div>`:''}`}
...
deleteProject=async function(name,retry=false){if(!retry&&!confirm(`Xóa toàn bộ dữ liệu local của dự án ${name}?`))return;const done=beginBusy(retry?'Đang tiếp tục...':'Đang bắt đầu...');try{const result=await api('/api/projects/'+encodeURIComponent(name)+'/delete',{method:'POST',body:JSON.stringify({confirmation:name}),timeoutMs:15000});closePanels();await refreshAll();toast(result.status==='completed'?'Dự án local đã được xóa':'Đã bắt đầu xóa local. Có thể reload trang an toàn.')}catch(e){toast(e.message,true)}finally{done()}}
'''
